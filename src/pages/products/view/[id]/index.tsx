...
import { deleteCartById } from 'apiSdk/carts';
...
function ProductViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ProductInterface>(
    ...
    () =>
      getProductById(id, {
        relations: ['vendor', 'cart', 'order'],
      }),
  );

  const cartHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('cart', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/carts/create?product_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.cart?.map((record) => (
                          <Tr cursor="pointer" onClick={() => router.push(`/carts/view/${record.id}`)} key={record.id}>
                            <Td>
                              {hasAccess('cart', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/carts/edit/${record.id}`}
                                >
